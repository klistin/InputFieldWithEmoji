...
/**
 * Unity 的InputField不支持emoji表情处理，所以单独写个支持emoji的InputField组件
 * emoji的编码比较特殊，Unicode编码大于2个字节，而中文一般来说是2个字节，英文一个字节
 * 这里面有以下几个难点
 * 1、因为存在emoji导致字符串的插入，移除，裁剪会比较困难得特殊处理
 * 2、光标的显示得处理
 * 3、高亮的显示得处理
 * 4、不同类型输入的情况得处理
 * 5、复杂的emoji表情因为存在零宽度连接符（\u200d）所以得特殊处理
*/
...
public class InputFieldWithEmoji : Selectable,
    ...
    ILayoutElement
{
    ...
    protected EmojiText m_TextComponent;
    ...
    protected int m_DrawStartWithEmoji = 0; //emoji参与了计算的开始位置
    protected int m_DrawStartPos = 0;
    protected int m_DrawEndWithEmoji = 0;//emoji参与了计算的结束位置
    protected int m_DrawEndPos = 0;
    ...
    void SetText(string value, bool sendCallback = true)
    {
        ...
        {
            m_Text = characterLimit > 0 && GetRealStrLength(value) > characterLimit ? SubStrWithEmoji(value,0, characterLimit) : value;
        }
        ...
        if (!Application.isPlaying)
        {
            ...
            int length = GetRenderStrLength(m_Text);
            if (m_CaretPosition > length)
                m_CaretPosition = m_CaretSelectPosition = length;
			else if (m_CaretSelectPosition > length)
                m_CaretSelectPosition = length;
        if (sendCallback)
            SendOnValueChanged();
        UpdateLabel();
    }
    ...
    /// </summary>
    public EmojiText textComponent
    {
    ...
    protected void ClampPos(ref int pos)
    {
        ...
        int length = GetRenderStrLength(text);
        if (pos > length) 
            pos = length;
    }
    ...
    protected override void OnEnable()
    {
        ...
        if (m_Text == null)
            m_Text = string.Empty;
        m_DrawStartWithEmoji = 0;
        m_DrawStartPos = 0;
        m_DrawEndWithEmoji = GetRealStrLength(m_Text);
        m_DrawEndPos = GetRenderStrLength(m_Text);
    ...
    {
        caretPositionInternal = GetRenderStrLength(text);
    ...
    {
        int position = GetRenderStrLength(text);
    ...
    protected virtual void LateUpdate()
    {
        ...
        if (m_Text != val)
        {
            ...
            else
            {
                ...
                var length = GetRealStrLength(m_Text);
                if (characterLimit > 0 && length > characterLimit)
                {
                    m_Text = SubStrWithEmoji(m_Text,0, characterLimit);
                }
                ...
                {
                    caretPositionInternal = caretSelectPositionInternal = GetRenderStrLength(m_Text);
				}
    ...
    public virtual void OnDrag(PointerEventData eventData)
    {
        ...
        var str = m_TextComponent.text;
        var charIndex = GetCharacterIndexFromPosition(localMousePos);
        var renderTxt = GetRenderStr(str); 
        //根据假的渲染的文本来算出真正显示字符数量
        var preRenderTxt = renderTxt.Substring(0, charIndex);
        caretSelectPositionInternal = preRenderTxt.Length + m_DrawStartPos;
        MarkGeometryAsDirty();
    ...
    public override void OnPointerDown(PointerEventData eventData)
    {
        ...
        if (hadFocusBefore)
        {
            ...
            var charIndex = GetCharacterIndexFromPosition(localMousePos);
            //点击到emoji表情了
            var renderTxt = m_TextComponent.emojiText;
            if (IsPositionBetweenReplaceTxt(renderTxt, charIndex))
                charIndex++;
            var tempStr = "";
            if (charIndex <= renderTxt.Length)
            {
                tempStr = renderTxt.Substring(0, charIndex);
            }
            else
            {
                tempStr = renderTxt;
                Debug.LogError("charIndex > renderTxt.Length");
            }

            //判断m_DrawStartPos是否换行到emoji了
            if (IsPositionBetweenReplaceTxt(GetRenderStr(text), m_DrawStartPos))
                m_DrawStartPos++;
            caretSelectPositionInternal = caretPositionInternal = tempStr.Length + m_DrawStartPos;
        }
        UpdateLabel();
        eventData.Use();
    }
    ...
    public virtual void OnUpdateSelected(BaseEventData eventData)
    {
        ...
        List<char> tempCharList = null; //用于存储emoji表情
        while (Event.PopEvent(m_ProcessingEvent))
        {
            if (m_ProcessingEvent.rawType == EventType.KeyDown)
            {
                //判断是不是emoji得字符
                if (char.IsSurrogate(m_ProcessingEvent.character))
                {
                ...
                if (tempCharList != null)
                {
                    tempCharList.Add(m_ProcessingEvent.character);
                    continue;
                }
                //防止编辑器输入emoji时输入不上
                if (m_ProcessingEvent.character == '\0' && Event.GetEventCount() > 0 &&
                    m_ProcessingEvent.keyCode == KeyCode.Backspace)
                {
                    continue;
                }
    ...
    private string GetSelectedString()
    {
        ...
        var caretPosWithEmoji = GetCaretPosWithEmoji(text, caretPositionInternal);
        var caretSelectPosWithEmoji = GetCaretPosWithEmoji(text, caretSelectPositionInternal);
        int startPos = caretPosWithEmoji;
        int endPos = caretSelectPosWithEmoji;
        ...
        return SubStrWithEmoji(text,startPos, endPos - startPos);
    }

    private int FindNextWordBegin()
    {
        int length = GetRenderStrLength(text);
        if (caretSelectPositionInternal + 1 >= length)
            return length;
    ...
    private void MoveRight(bool shift, bool ctrl)
    {
        ...
        {
            var caretPosWithEmoji = GetCaretPosWithEmoji(text, caretSelectPositionInternal);
            var caretPos = GetCaretPos(text, caretPosWithEmoji + 1);
            position = caretPos;
        }
    ...
    private int DetermineCharacterLine(int charPos, TextGenerator generator)
    {
        ...
        {
            var nextLine = generator.lines[i + 1];
            if (nextLine.startCharIdx > charPos)
                return i;
        }
    ...
    private int LineDownCharacterPosition(int originalPos, bool goToLastChar)
    {
        if (originalPos >= cachedInputTextGenerator.characterCountVisible)
            return GetRenderStrLength(text);
        ...
        if (originLine + 1 >= cachedInputTextGenerator.lineCount)
            return goToLastChar ? GetRenderStrLength(text) : originalPos;
    ...
    private void MoveDown(bool shift, bool goToLastChar)
    {
        ...
        int position = multiLine ? LineDownCharacterPosition(caretSelectPositionInternal, goToLastChar) : GetRenderStrLength(text);
        if (IsPositionBetweenReplaceTxt(GetRenderStr(text), position))
            position++;
        if (shift)
            caretSelectPositionInternal = position;
        else
    ...
    private void Delete()
    {
        ...
        if (caretPositionInternal == caretSelectPositionInternal)
            return;
        var totalLength = GetRealStrLength(text);
        var caretPosWithEmoji = GetCaretPosWithEmoji(text, caretPositionInternal);
        var caretSelectPosWithEmoji = GetCaretPosWithEmoji(text, caretSelectPositionInternal);
        if (caretPositionInternal < caretSelectPositionInternal)
        {
            if (caretSelectPosWithEmoji < totalLength)
            {
                m_Text = SubStrWithEmoji(text,0, caretPosWithEmoji) + SubStrWithEmoji(text,caretSelectPosWithEmoji, totalLength - caretSelectPosWithEmoji);
            }
            else
            {
                m_Text = SubStrWithEmoji(text,0, caretPosWithEmoji);
            }
        ...
        {
            if (caretPosWithEmoji < totalLength)
            {
                //删除中间一段字符串
                m_Text = SubStrWithEmoji(text,0, caretSelectPosWithEmoji) + SubStrWithEmoji(text,caretPosWithEmoji, totalLength- caretPosWithEmoji);
            }
            else
            {
                //删除末尾的字符串
                m_Text = SubStrWithEmoji(text,0, caretSelectPosWithEmoji);
            }
    ...
    private void ForwardSpace()
    {
        ...
        else
        {
            ...
            {
                m_Text = RemoveRealStr(text,caretPositionInternal,1);
				UpdateTouchKeyboardFromEditChanges();
                SendOnValueChangedAndUpdateLabel();
            }
        }
    ...
    private void Backspace()
    {
        ...
        else
        {
            ...
            {
                var caretPosWithEmoji = GetCaretPosWithEmoji(text, caretPositionInternal);
                m_Text = RemoveRealStr(text,caretPosWithEmoji,1);
                var caretPos = GetCaretPos(text, caretPosWithEmoji - 1);
                caretSelectPositionInternal = caretPositionInternal = caretPos;
				UpdateTouchKeyboardFromEditChanges();
                SendOnValueChangedAndUpdateLabel();
            }
        }
    ...
    private void Insert(char[] charArr)
    {
        ...
        string replaceString = string.Concat(charArr);
        Delete();
        var charLength = charArr.Length;
        // Can't go past the character limit
        if (characterLimit > 0 && charLength == 1 && text.Length >= characterLimit)
            return;
        else if(characterLimit > 0 && charLength > 1 && GetRealStrLength(text) >= characterLimit)
            return;

        var caretPosWithEmoji = GetCaretPosWithEmoji(text,m_CaretPosition);
        m_Text = InsertRealStr(text,caretPosWithEmoji, replaceString);
        caretSelectPositionInternal = caretPositionInternal += GetRenderStrLength(replaceString);
    ...
    protected virtual void Append(string input)
    {
        ...
        if (!InPlaceEditing())
            return;
        EmojiUtils.HandleTextWithCallbacks(input,Append,Append);
    }
    ...
    protected virtual void Append(char[] inputChatArr)
    {
        ...
        if (inputChatArr.Length == 1)
        {
            var input = inputChatArr[0];
        ...
        else if (characterValidation != CharacterValidation.None || inputType == InputType.Password)
        {
            return;
        }
    ...
    protected void UpdateLabel()
    {
        if (m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback)
        {
            ...
            if (compositionString.Length > 0)
            {
                ...
                {
                    fullText = compositionString;
                }
                else
                {
                    var tempPos =  GetCaretPosWithEmoji(text,m_CaretPosition);
                    if (tempPos < GetRealStrLength(text))
                    {
                        fullText = SubStrWithEmoji(text,0, tempPos) + compositionString + SubStrWithEmoji(text,tempPos);
                    }
                    else
                    {
                        fullText = SubStrWithEmoji(text,0, tempPos) + compositionString;
                    }
                }
            ...
            string processed;
            if (inputType == InputType.Password)
                processed = new string(asteriskChar, GetRealStrLength(fullText));
            else
                processed = fullText;
            ...
            if (!isEmpty)
            {
                ...
                var renderTxt = GetRenderStr(processed);
                cachedInputTextGenerator.PopulateWithErrors(renderTxt, settings,gameObject);
                //计算绘制的范围
                SetDrawRangeToContainCaretPosition(caretSelectPositionInternal,renderTxt);

                // var minEndPos = Mathf.Min(m_DrawEndWithEmoji, GetRealStrLength(processed));
                
                //得到显示的文本
                if(m_DrawStartWithEmoji <= m_DrawEndWithEmoji)
                    processed = SubStrWithEmoji(processed,m_DrawStartWithEmoji, m_DrawEndWithEmoji - m_DrawStartWithEmoji);
                else 
                    Debug.LogError("m_DrawStartWithEmoji > m_DrawEndWithEmoji");
    ...
    //获取渲染的假的字符串长度（带%？）
    private int GetRenderStrLength(string str)
    {
        return GetRenderStr(str).Length;
    }

    //获取渲染的文本
    private string GetRenderStr(string str)
    {
        if (!string.IsNullOrEmpty(str))
        {
            //为了防止原文中包含替换符影响后面的计算所以这里特殊处理下
            if (str.Contains(EmojiText.ReplacementTxt))
            {
                //模式 "%\\?" 中的 \\ 是为了转义 "?"，因为 "?" 在正则表达式中有特殊的含义
                str = Regex.Replace(str, "%\\?", "**");
            }

            return EmojiUtils.GetRenderStr(str);
        }
        return str;
    }

    //获取字符串长度
    private int GetRealStrLength(string str)
    {
        int length = 0;
        EmojiUtils.HandleTextWithCallbacks(str, () =>
        {
            length++;
        },(nonEmojiStr) =>
        {
    ...
    //移除指定长度的字符串
    private string RemoveRealStr(string str,int posWithEmoji,int length)
    {
       int totalLength = GetRealStrLength(str);
        if (posWithEmoji <= totalLength)
        {
            //删除位置在中间
            if (length < posWithEmoji)
            {
                if (posWithEmoji < totalLength)
                {
                    return SubStrWithEmoji(str,0,posWithEmoji - length)+ SubStrWithEmoji(str,posWithEmoji);
                }
                else
                {
                    //pos == totalLength 从尾部删除length个字符
                    return SubStrWithEmoji(str,0,posWithEmoji - length);
                }
            }
            else
            {
                ...
                if (posWithEmoji < totalLength)
                {
                    return SubStrWithEmoji(str, posWithEmoji);
                }
                return "";
    ...
    //插入文本
    private string InsertRealStr(string str,int posWithEmoji,string subStr)
    {
        if (string.IsNullOrEmpty(str))
            return subStr;
        if (posWithEmoji < GetRealStrLength(str))
        {
            //插入字符串头
            if (posWithEmoji == 0) 
                return subStr + str;
            return SubStrWithEmoji(str,0,posWithEmoji)+ subStr + SubStrWithEmoji(str,posWithEmoji);
        }
        else
        {
            //插入字符串尾
    ...
    private string SubStrWithEmoji(string str, int startPosWithEmoji)
    {
        return SubStrWithEmoji(str, startPosWithEmoji, -1);
    }
    ...
    /// <param name="startPosWithEmoji">从第几个字符开始</param>
    /// <param name="length">截取的长度</param>
    /// <returns>返回截取后的字符串</returns>
    private string SubStrWithEmoji(string str,int startPosWithEmoji,int length)
    {
        if (length == 0)
            return "";
        var strInfo = new StringInfo(str);
        // var strEnumerator = StringInfo.GetTextElementEnumerator(str);
        //startIndex 和 endIndex 用于真正的裁剪参数
        int startIndex = -1;
        int endIndex = length == - 1? strInfo.LengthInTextElements:-1;
        int charNum = 0;
        int charIndex = 0;
        int endPosWithEmoji = length == - 1? -1:startPosWithEmoji + length;
        // bool hasCountryFlagEmoji = false;
        // while (strEnumerator.MoveNext())
        // {
        //     if (startIndex == -1 && charNum == startPosWithEmoji)
        //         startIndex = charIndex;
        //     if (endIndex == -1 && charNum == endPosWithEmoji)
        //     {
        //         endIndex = charIndex;
        //         break;
        //     }
        ...
        //     // 跳过控制字符
        //     if (EmojiUtils.NextCharIsControl(str, strEnumerator))
        //     {
        //         hasCountryFlagEmoji = false;
        //         continue;
        //     }
        //     
        //     // 处理国旗标志符号
        //     if (!hasCountryFlagEmoji && EmojiUtils.NextCharIsCountryIndicator(str, strEnumerator))
        //     {
        //         hasCountryFlagEmoji = true;
        //         continue;
        //     }
        //     // 跳过零宽连接符
        //     if (EmojiUtils.CharIsZWJ(charArr[0]))
        //     {
        //         hasCountryFlagEmoji = false;
        //         continue;
        //     }
        ...
        EmojiUtils.HandleTextWithCallbacks(str, null,null, () =>
        {
            if (charNum == endPosWithEmoji)
                return true;
            charNum++;//真实的字符计数
            return false;
        }, () =>
        ...
        if (endIndex == -1)
            endIndex = strInfo.LengthInTextElements;
        
        if(startIndex < 0)
            Debug.LogError($"startIndex is negative number,str = {str},startPosWithEmoji = {startPosWithEmoji},length = {length}");
        if(startIndex >= endIndex)
            Debug.LogError($"startIndex >= endIndex ,str = {str},startPosWithEmoji = {startPosWithEmoji},length = {length}");
        // 返回裁剪后的子字符串
        return strInfo.SubstringByTextElements(startIndex, endIndex - startIndex);
    }

    //获取不带emoji计算的游标真实的位置
    private int GetCaretPos(string str,int caretPosWithEmoji)
    {
        if (string.IsNullOrEmpty(str) || caretPosWithEmoji == 0)
            return 0;
        int startIndex = 0;
        int emojiNum = 0;
        ...
        {
            startIndex++;
            if (startIndex >= caretPosWithEmoji)
                return true;
            return false;
        });

        return caretPosWithEmoji + emojiNum;
    }
    
    
    //获取参与了emoji计算的游标的位置
    private int GetCaretPosWithEmoji(string str,int caretPos)
    {
        if (string.IsNullOrEmpty(str))
            return 0;
        int curIndex = caretPos;
        int startIndex = 0;
        int txtNum = 0;
        EmojiUtils.HandleTextWithCallbacks(str, () =>
        {
    ...
    {
        if (m_DrawStartWithEmoji > caretPositionInternal || m_DrawStartWithEmoji > caretSelectPositionInternal)
            return false;

        if (m_DrawEndWithEmoji < caretPositionInternal || m_DrawEndWithEmoji < caretSelectPositionInternal)
            return false;
    ...
    private void SetDrawRangeToContainCaretPosition(int caretPos,string renderTxt)
    {
        ...
        if (multiLine)
        {
            ...
            if (caretPos >= m_DrawEndPos)
            {
                // Caret comes after drawEnd, so we need to move drawEnd to the end of the line with the caret
                m_DrawEndPos = GetLineEndPosition(cachedInputTextGenerator, caretLine);
                ...
                m_DrawStartPos = GetLineStartPosition(cachedInputTextGenerator, startLine);
            }
            else
            {
                if (caretPos < m_DrawStartPos)
                {
                    // Caret comes before drawStart, so we need to move drawStart to an earlier line start that comes before caret.
                    m_DrawStartPos = GetLineStartPosition(cachedInputTextGenerator, caretLine);
                }

                int startLine = DetermineCharacterLine(m_DrawStartPos, cachedInputTextGenerator);
                ...
                m_DrawEndPos = GetLineEndPosition(cachedInputTextGenerator, endLine);
                while (startLine > 0)
                {
        ...
        else
        {
            ...
            if (m_DrawEndPos > cachedInputTextGenerator.characterCountVisible)
                m_DrawEndPos = cachedInputTextGenerator.characterCountVisible;

            float width = 0.0f;
            if (caretPos > m_DrawEndPos || (caretPos == m_DrawEndPos && m_DrawStartPos > 0))
            {
                // fit characters from the caretPos leftward
                m_DrawEndPos = caretPos;
                for (m_DrawStartPos = m_DrawEndPos - 1; m_DrawStartPos >= 0; --m_DrawStartPos)
                {
                    if (width + characters[m_DrawStartPos].charWidth > extents.x)
                        break;

                    width += characters[m_DrawStartPos].charWidth;
                }
                ++m_DrawStartPos;  // move right one to the last character we could fit on the left
            }
            else
            {
                if (caretPos < m_DrawStartPos)
                    m_DrawStartPos = caretPos;

                m_DrawEndPos = m_DrawStartPos;
            }

            // fit characters rightward
            for (; m_DrawEndPos < cachedInputTextGenerator.characterCountVisible; ++m_DrawEndPos)
            {
                width += characters[m_DrawEndPos].charWidth;
                if (width > extents.x)
                    break;
            }
        }
        var tempStr = "";
        if (m_DrawStartPos <= renderTxt.Length)
        {
            tempStr = renderTxt.Substring(0, m_DrawStartPos);
        }
        else
        {
            tempStr = renderTxt;
            Debug.LogError("m_DrawStartPos > renderTxt.Length");
        }

        m_DrawStartWithEmoji = m_DrawStartPos - CalcReplaceTxtNum(tempStr);
        
        if (m_DrawEndPos <= renderTxt.Length)
        {
            tempStr = renderTxt.Substring(0, m_DrawEndPos);
        }
        else
        {
            tempStr = renderTxt;
            Debug.LogError("m_DrawEndPos > renderTxt.Length");
        }
        m_DrawEndWithEmoji = m_DrawEndPos - CalcReplaceTxtNum(tempStr);
        if (m_DrawEndWithEmoji < 0)
        {
            Debug.LogError("m_DrawEnd < 0");
            m_DrawEndWithEmoji = 0;
        }
    }

    /// <summary>
    /// 指定位置pos,想看下这个pos是否处于"%？"之间
    /// </summary>
    /// <param name="preRenderText">cachedInputTextGenerator 预渲染的文本</param>
    /// <param name="pos">指定的位置</param>
    private bool IsPositionBetweenReplaceTxt(string preRenderText,int pos)
    {
        int index = 0;

        // 在字符串中循环查找"%?"
        while ((index = preRenderText.IndexOf(EmojiText.ReplacementTxt, index, StringComparison.Ordinal)) != -1)
        {
            // 如果指定位置在当前 "%?" 的起始和结束位置之间，返回 true
            if (index < pos && pos <= index + 1)
            {
                return true;
            }
            index += EmojiText.ReplacementTxt.Length; 
        }

        // 没有找到匹配或者指定位置不在任何 "%?" 之间，返回 false
        return false;
    }

    /// <summary>
    /// 计算替换EmojiText.ReplacementTxt的数量
    /// </summary>
    /// <param name="preRenderText">cachedInputTextGenerator 预渲染的文本</param>
    private int CalcReplaceTxtNum(string preRenderText)
    {
        // 初始位置
        int index = 0;
        int count = 0;

        // 在字符串中循环查找EmojiText.ReplacementTxt
        while ((index = preRenderText.IndexOf(EmojiText.ReplacementTxt, index, StringComparison.Ordinal)) != -1)
        {
            index += EmojiText.ReplacementTxt.Length;
            count++;
    ...
    private void GenerateCaret(VertexHelper vbo, Vector2 roundingOffset)
    {
        ...
        int adjustedPos = Mathf.Max(0, caretPositionInternal - m_DrawStartPos);
        TextGenerator gen = m_TextComponent.cachedTextGenerator;
    ...
    {
        int startChar = Mathf.Max(0, caretPositionInternal - m_DrawStartPos);
        int endChar = Mathf.Max(0, caretSelectPositionInternal - m_DrawStartPos);
    ...
    protected char Validate(string text, int pos, char ch)
    {
        ...
        if (characterValidation == CharacterValidation.None || !enabled)
            return ch;
        //判断一下长度，如果长度不一样说明包含emoji，直接返回
        if (GetRealStrLength(text) != text.Length)
            return (char)0;
